...
export function makeFerry(opts: { shocks: Record<ShockKey, boolean>; policyMonth: number }): FerryPoint[] {
  ...
  for (let t = 0; t < 84; t++) {
    ...
    const season = 8 * Math.sin(((month - 3) / 12) * 2 * Math.PI);
    const festival = 3 * Math.exp(-Math.pow(month - 9, 2) / 2); // 渔获节前后的轻微平滑隆起
...
export function makeBlocks(): Block[] {
  ...
  const spec: Array<Omit<Block, "prices">> = [
    ...
    { key: "hx", name: "河西", treated: false, density: 205, distance: 4.5, prePrice: 2.4, note: "水平比通车街区低，但通车前涨得一样快" },
    { key: "nw", name: "南屋", treated: false, density: 191, distance: 5.4, prePrice: 2.2 },
    { key: "tq", name: "桐渠", treated: false, density: 224, distance: 3.9, prePrice: 2.6 },
    { key: "cl", name: "长垒", treated: false, density: 179, distance: 6.1, prePrice: 2.0 },
  ...
  return spec.map((b) => {
    ...
    DID_YEARS.forEach((y, i) => {
      ...
      if (b.treated && y >= OPEN_YEAR) p += 0.65 * Math.min(1, (y - OPEN_YEAR + 1) / 2);
