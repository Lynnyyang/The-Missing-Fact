...
5. 回答要短。总共不超过 120 个字，分三段，段间空行，每段一到两句：
   第一段：读出当前界面上最关键的一个或两个数字，说它意味着什么。
   第二段：结合最近操作，指出这一步的关键或最容易误读的地方。
   第三段：点名一个具体控件，说下一步怎么动、会看到什么变化。
6. 全中文，不用列表符号、不用标题号、不写段落标题；每段最多用一处双星号标重点，例如 **随机分签**，其它地方不要出现星号。
7. 提到控件时，只能用「界面状态」里出现的中文名称原样称呼，不得自己造按钮名或菜单名。
8. 不说空话套话、不复述规则、不写开场问候和结尾总结。`;
